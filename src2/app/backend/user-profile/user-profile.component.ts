...
export class UserProfileComponent implements OnInit {
  ...
  firebaseData(){  
    this.db.object('atwk_user_info/'+this.userId).valueChanges().subscribe(
      (data:Profile)=>{
        ...
        console.log(this.profile.UserTimeZone)   
        this.profile.LocalOffset= this.timezoneList.filter(x=>x.timezone==this.profile.UserTimeZone)[0].offset ;
        this.profile.LocalTime =new Date((new Date().getTime() + new Date().getTimezoneOffset()*1000 )+this.profile.LocalOffset*1000);
  ...
  getConversation(data){
    let temp = {email:data.From,ayatollah: data.ayatollah,subject: data.Subject,toEmail: data.To,}

    if (this.as.getLocalStorage().email == temp.email.toLowerCase()) {
      temp.email= temp.toEmail;
    }else{
      temp.email= temp.email;
    }
    this.userMessageHistoryService.messageHistoryFun(temp);
